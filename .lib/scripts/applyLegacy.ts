...
// All role folders (boreal, brav, provecto) configure the same role key on
// their respective Roles mods — folder names don't map to role keys here.
...
async function main() {
  ...
  const ownerAddress = await fetchModOwner(chainId, address);
  if (!ownerAddress) {
    console.warn(`Could not read owner() from Roles mod at ${modArg}`);
  }
